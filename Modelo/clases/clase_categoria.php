<?php
/**
 * Clase para realizar operaciones respecto a categoría en la base de datos.
 */
class categoria{
    /**
    ...
    */
    public function insertar_categoria($nombre, $num_equipos, $estado, $id_liga, $conexion) {
        // Inicializar variable de retorno.
        $respuesta = false;

        // Crear la sentencia SQL con una consulta preparada.
        $sql = "INSERT INTO categoria(nombre, num_equipos, estado, id_Liga) VALUES (?,?,?,?)";
        $stmt = mysqli_prepare($conexion, $sql);

        if($stmt){
            // Vincular parámetros a la sentencia.
            mysqli_stmt_bind_param($stmt, "sisi", $nombre, $num_equipos, $estado, $id_liga);
    ...
    */
    function mostrar_categorias($id_liga, $conexion) {
        // Inicializar variable de retorno.
        $categorias = array();
    ...
    */
    public function actualizar_categoria($nombre, $num_equipos, $id_categoria, $conexion){
        // Inicializar variable de retorno
        $respuesta = false;

        // Crear la sentencia SQL con una consulta preparada.
        $sql = "UPDATE categoria SET nombre = ?, num_equipos = ? WHERE id_categoria= ?";
        $stmt = mysqli_prepare($conexion, $sql);

        if ($stmt) {
            // Vincular el parámetro a la sentencia
            mysqli_stmt_bind_param($stmt, "sii", $nombre, $num_equipos, $id_categoria);
        ...
        // Devolver `true` si se actualizó la categoría, `false` en caso contrario.
        return $respuesta;
    }
    ...
    */
    public function eliminar_categoria($id_categoria,$conexion){
        // Inicializar variable de retorno
        $respuesta = false;
        ...
        // Devolver `true` si actualizo la categoría en estado inactivo, `false` en caso contrario.
        return $respuesta;
    }
}
?>